...
from llava_utils.data_utils import load_yaml, construct_prompt, save_json, process_single_sample
from llava_utils.model_utils import call_llava_engine_df, llava_image_processor
from llava_utils.eval_utils import parse_multi_choice_response
...
def eval_mmmu_results(output_path, answer_path):
    ...
    print('Overall-Art and Design:', printable_results['Overall-Art and Design'])
    print('Overall-Business:', printable_results['Overall-Business'])
    print('Overall-Science:', printable_results['Overall-Science'])
    print('Overall-Health and Medicine:', printable_results['Overall-Health and Medicine'])
    print('Overall-Humanities and Social Science:', printable_results['Overall-Humanities and Social Science'])
    print('Overall-Tech and Engineering:', printable_results['Overall-Tech and Engineering'])
    print('Overall:', printable_results['Overall'])
...
def test_mmmu(args,tokenizer, model, vis_processors, test_data_path):
    ...
    config = load_yaml('llava_utils/llava1.5.yaml')
    ...
    for subject in CAT_SHORT2LONG.values():
        sub_dataset = load_dataset(test_data_path, subject, split='validation')
    ...
    for sample in tqdm(dataset):
        ...
        if "validation" in sample['id']:
            sample = construct_prompt(sample, config)
            # samples.append(sample)
            with torch.no_grad():
                if sample['image']:
                    sample['image'] = vis_process_func(sample['image'], vis_processors)
