...
from model_merging_methods.mask_weights_utils import mask_model_weights, _down_project, _up_project,delta_parameter_compression
...
class MergingMethod:
    ...
    def low_dim_merging(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list,rank:int, scaling_coefficient: float = 1.0):
        """
        task arithmetic method
        :param merged_model: nn.Module, the merged model
        :param models_to_merge: list, individual models that need to be merged
        ...
        print("You are using Low Dim Merging!!!!")


        assert isinstance(scaling_coefficient, float), "wrong type of scaling_coefficient, should be float!"

        models_to_merge_task_vectors_param_dict = [TaskVector(pretrained_model=merged_model,
                                                   finetuned_model=model_to_merge,
                                                   exclude_param_names_regex=exclude_param_names_regex).task_vector_param_dict for model_to_merge in models_to_merge]

        param_names_to_merge = get_param_names_to_merge(input_param_names=list(models_to_merge_task_vectors_param_dict[0].keys()),
        ...
        with torch.no_grad():
            ...
            for param_name in tqdm(param_names_to_merge):
                ...
                seed = torch.randint(low=0, high=999, size=(1,)).item()

                projected_param_1 = _down_project(seed=seed, grad=models_to_merge_task_vectors_param_dict[0][param_name], proj_rank=rank)

                projected_param_2 = _down_project(seed=seed, grad=models_to_merge_task_vectors_param_dict[1][param_name], proj_rank=rank)

                merged_param_tensor = scaling_coefficient * projected_param_1 + (1-scaling_coefficient) * projected_param_2


                recoverd_mat = _up_project(seed=seed,
                                           param=models_to_merge_task_vectors_param_dict[0][param_name],
                                           compressed_grad=merged_param_tensor,
                                           proj_rank=rank)

                merged_task_vector[param_name] = recoverd_mat

            new_task_vector = TaskVector(task_vector_param_dict=merged_task_vector)
            # combine with parameters of the merged model based on scaling coefficient
            merged_params = new_task_vector.combine_with_pretrained_model(pretrained_model=merged_model,
                                                                              scaling_coefficient=1.0)

            return merged_params


    def fisher_merging(self, models_to_merge: list,
                       trainers: list,
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list, trainers: list = None, scaling_coefficient: float = 1.0,
        ...
        elif self.merging_method_name == "low_dim":
            merged_params = self.low_dim_merging(merged_model=merged_model, models_to_merge=models_to_merge, exclude_param_names_regex=exclude_param_names_regex,
                                                 rank=8192,scaling_coefficient=scaling_coefficient)
