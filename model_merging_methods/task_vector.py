...
class TaskVector:
    def __init__(self, pretrained_model: nn.Module = None, finetuned_model: nn.Module = None, exclude_param_names_regex: list = None, task_vector_param_dict: dict = None):
        ...
        if task_vector_param_dict is not None:
            ...
            if len(finetuned_param_dict)< len(pretrained_param_dict):
                param_names_to_merge = get_param_names_to_merge(input_param_names=list(finetuned_param_dict.keys()), exclude_param_names_regex=exclude_param_names_regex)
                with torch.no_grad():
                    for param_name in param_names_to_merge:
                        if param_name in finetuned_param_dict.keys():
                            self.task_vector_param_dict[param_name] = finetuned_param_dict[param_name] - pretrained_param_dict[param_name]
                        else:
                            self.task_vector_param_dict[param_name] = - pretrained_param_dict[param_name]
    ...
    def combine_with_pretrained_model_v2(self, pretrained_param_dict, llm_param_dict, scaling_coefficient: float = 1.0):
        """
        combine the task vector with pretrained model
        :param pretrained_model: nn.Module, pretrained model
        :param scaling_coefficient: float, scaling coefficient to merge the task vector
        ...
        with torch.no_grad():
            merged_params = {}
            for param_name in self.task_vector_param_dict:
                if param_name in llm_param_dict.keys():
                    merged_params[param_name] = pretrained_param_dict[param_name] + scaling_coefficient * self.task_vector_param_dict[param_name] + scaling_coefficient * llm_param_dict[param_name]
    ...
    def combine_with_pretrained_model_v3(self, pretrained_param_dict, llm_param_dict, fine_tuned_llm, scaling_coefficient: float = 1.0):
        """
        combine the task vector with pretrained model
        :param pretrained_model: nn.Module, pretrained model
        :param scaling_coefficient: float, scaling coefficient to merge the task vector
        ...
        with torch.no_grad():
            ...
            for param_name in self.task_vector_param_dict:
                if param_name in llm_param_dict.keys():
                    merged_params[param_name] = fine_tuned_llm[param_name]
    ...
    def combine_with_pretrained_model_v4(self, pretrained_model: nn.Module,pretrained_llm: nn.Module,  scaling_coefficient: float = 1.0):
        """
        combine the task vector with pretrained model
        :param pretrained_model: nn.Module, pretrained model
        :param scaling_coefficient: float, scaling coefficient to merge the task vector
        ...
        with torch.no_grad():
            ...
            #     pretrained_param_dict[param_name] = delta_parameter_compression(param_value)
            merged_params = {}

            for param_name in self.task_vector_param_dict:
                pretrained_param_dict[param_name] = pretrained_param_dict[param_name] + self.task_vector_param_dict[
                    param_name]
    ...
    def combine_with_pretrained_model_v5(self, pretrained_model: nn.Module,pretrained_llm: nn.Module,  scaling_coefficient: float = 1.0):
        """
        combine the task vector with pretrained model
        :param pretrained_model: nn.Module, pretrained model
        :param scaling_coefficient: float, scaling coefficient to merge the task vector
        ...
        # pretrained_llm_dict = {param_name: param_value for param_name, param_value in pretrained_llm.named_parameters()}
        ...
        with torch.no_grad():
            ...
            for param_name in self.task_vector_param_dict:
                ...
                pretrained_param_dict[param_name] = pretrained_param_dict[param_name] + self.task_vector_param_dict[param_name]
    ...
    def combine_with_pretrained_model_v6(self, pretrained_model: nn.Module,pretrained_llm: nn.Module, pm_list, scaling_coefficient: float = 1.0):
        """
        combine the task vector with pretrained model
        :param pretrained_model: nn.Module, pretrained model
        :param scaling_coefficient: float, scaling coefficient to merge the task vector
        ...
        with torch.no_grad():
            for param_name in self.task_vector_param_dict:
                ...
                if param_name in pm_list:
                    pretrained_param_dict[param_name] = pretrained_llm_dict[param_name]+self.task_vector_param_dict[param_name]


        return pretrained_param_dict


    def combine_with_pretrained_model_v7(self, pretrained_model: nn.Module,pretrained_llm: nn.Module,  scaling_coefficient: float = 1.0):
        """
        combine the task vector with pretrained model
        :param pretrained_model: nn.Module, pretrained model
        :param scaling_coefficient: float, scaling coefficient to merge the task vector
        ...
        with torch.no_grad():
            for param_name in self.task_vector_param_dict:
                if 'self_attn' in param_name or 'mlp' in param_name:
                    pretrained_param_dict[param_name] = pretrained_llm_dict[param_name]+scaling_coefficient*self.task_vector_param_dict[param_name]
